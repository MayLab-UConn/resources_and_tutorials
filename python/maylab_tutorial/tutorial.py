...
# better autocomplete
# magic
# can use simple os commands
# -   timing

''' The python language:
        -comments start with #
        -long comments are encapsulated with triple quotation marks
        -no semicolons needed at end of line, but multiple statements on the
         same line must be separated by a semicolons
        -functions, loops, conditional blocks are defined by the indentation
'''
# -----------------------------------------------------------------------------
...
np_list
boolean_list = np_list < 5  # can't do this with normal lists
np_list.shape
boolean_list.shape
...
'''There are a lot of ways to input/output files, the basic python implementation
    uses open, read, and write
'''
input_file = open('sample_io_input.txt', 'r')  # the r stands for read
text = input_file.read()
output_file = open('sample_io_output.txt', 'w')  # the w stands for write
output_file.write('Hello world \nSecond line')
output_file.write('Third line')
input_file.close()
output_file.close()
# double vs single quotes


''' numpy has its own i/o tools for converting text data to an array, which
...
in_angles =  np.loadtxt('angles_itp_orig.txt', dtype=int)
...
replace_column_indices('bonds_itp_orig.txt'    , 'pairs_out.txt'    , number_mapping_dict, [1, 2])
replace_column_indices('pairs_itp_orig.txt'    , 'bonds_out.txt'    , number_mapping_dict, [1, 2])
replace_column_indices('angles_itp_orig.txt'   , 'angles_out.txt'   , number_mapping_dict, [1, 2, 3])
replace_column_indices('dih_itp_orig.txt'      , 'dihedrals_out.txt', number_mapping_dict, [1, 2, 3, 4])
...
# lets try again, now including all molecules that are partially in the slap
...
''' Some general coding advice:

    - use descriptive cariable names, except for sometimes in loops
    - if a piece of code is repeated, make it a function and call the function
        - sometimes do that even if it isn't repeated, can add clarity
    - google EVERYTHING
    - test as you go!
